import * as icons from "simple-icons";

export type Project = {
  name: string;
  description: string;
  url: string;
  github?: string;
  technologies: Technology[];
};

export type Technology =
  | "js"
  | "ts"
  | "rust"
  | "react"
  | "next"
  | "roblox"
  | "interface"
  | "luau";

export type TechnologyInfo = {
  name: string;
  icon: string;
};

export const technologies: Record<Technology, TechnologyInfo> = {
  js: {
    name: "JS",
    icon: icons.siJavascript.path,
  },
  ts: {
    name: "TS",
    icon: icons.siTypescript.path,
  },
  rust: {
    name: "Rust",
    icon: icons.siRust.path,
  },
  react: {
    name: "React",
    icon: icons.siReact.path,
  },
  next: {
    name: "Next.js",
    icon: icons.siNextdotjs.path,
  },
  roblox: {
    name: "Roblox",
    icon: icons.siRoblox.path,
  },
  interface: {
    name: "Interface",
    icon: icons.siMaterialdesignicons.path,
  },
  luau: {
    name: "Luau",
    icon: "M5.0718 0L24 5.0718L18.9282 24L0 18.9282L5.0718 0ZM14.2365 4.99156L21 6.80383L19.1877 13.5673L12.4243 11.755L14.2365 4.99156Z",
  },
};

export const projects: Project[] = [
  {
    name: "Bow Simulator",
    description: "A Simulator Game. Where have to tirer à la l'arc pour gagner de l'argent.",
    url: "https://www.roblox.com/games/15395018036/Bow-Simulator",
    technologies: ["interface"],
  },
  {
    name: "Game RNG",
    description: "A RNG Game. Where your roll thousands of famous roblox expériences.",
    url: "https://www.roblox.com/games/17051457409/Game-RNG",
    technologies: ["interface", "luau"],
  },
];
